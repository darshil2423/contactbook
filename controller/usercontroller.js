...
exports.login = async (req, res) => {
    try {
        ...
        var users_id = await storage.getItem('user_id');
    ...
    try {
        ...
        if (!userId) {
            return res.status(401).json({
                status: 'false',
                message: 'You must be logged in to add a contact'
            });
        }
        else {
...
exports.viewcontact = async (req, res) => {
    try {
        ...
        if (!userId) {
            ...
            var data = await contact.find({ user_id: req.body.user_id }).populate('user_id');
            res.status(200).json({
                status: 'true',
